...
KMCModule.config(['$routeProvider', '$locationProvider', '$httpProvider', '$tooltipProvider', '$translateProvider', function ($routeProvider, $locationProvider, $httpProvider, $tooltipProvider, $translateProvider) {
...
}]).run(["$rootScope", "$rootElement", "$location", "$window", "$translate", function ($rootScope, $rootElement, $location, $window, $translate) {
    ...
    var item = $window.localStorage.getItem("ngx-webstorage|kmc_lang");
    if (item) {
        if (item === '"fa"') {
            $translate.use("fa_IR");
        }
