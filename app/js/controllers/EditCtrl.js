...
KMCMenu.controller('EditCtrl', ['$scope', '$http', '$timeout', 'PlayerData', 'PlayerService', 'apiService', 'editableProperties', 'localStorageService', '$routeParams', '$modal', '$location', 'requestNotificationChannel', 'select2Svc', 'utilsSvc', '$window',
    function ($scope, $http, $timeout, PlayerData, PlayerService, apiService, editableProperties, localStorageService, $routeParams, $modal, $location, requestNotificationChannel, select2Svc, utilsSvc, $window) {
        ...
        var lang = $window.localStorage.getItem("ngx-webstorage|kmc_lang") || $window.localStorage.getItem("ls.NG_TRANSLATE_LANG_KEY");
        $scope.layout = {
            rtl: (lang === '"fa"' || lang === 'fa_IR'),
            move: $scope.menuOpen ? '439px' : '10px',
            margin: $scope.menuOpen ? '490px' : '50px'
        };
        ...
        editableProperties.then(function (data) {
            /*console.log("---------------------------------------------");
            var lang = $window.localStorage.getItem("ngx-webstorage|kmc_lang");
            console.log("--------",lang);
            console.log(data);
            if (lang === '"fa"')
                data = angular.copy(data.fa); // prevent changing original data
            else
                data = angular.copy(data.en);*/ // prevent changing original data
            ...
            $scope.menuData = [{
                ...
                'icon': 'TabSearch',
                'properties': [{'type': 'search', 'model': 'menuProperties'}]
            }];
            ...
            for (var cat in data) {
                ...
                for (var plug in plugs) {
                    ...
                    if (p.children === undefined) { // this is a flat object like a Flashvar and not a plugin
                        ...
                        }); // add accordion header to the search indexing
                        var plugin = {
                            'enabled': p.enabled,
                            'label': p.label,
                            ...
                            if (p.sections && p.children[i].section) { // property should be put in the correct tab
                                for (var t = 0; t < tabObj.children.length; t++) {
                                    if (p.children[i].section == tabObj.children[t].key) {
                                        ...
                                        }); // add property to search indexing
                                    }
                                }
